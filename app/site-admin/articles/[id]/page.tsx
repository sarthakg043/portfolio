    ...
    <AdminShell email={user.email ?? "Administrator"}>
      <main className="mx-auto max-w-7xl px-5 py-8 sm:px-8">
        <div className="mb-8 flex items-center justify-between gap-4">
          <Link href={getAdminPath("/articles")} className="inline-flex items-center gap-2 text-sm text-neutral-500 hover:text-neutral-950 dark:hover:text-white"><ArrowLeft className="size-4" /> Articles</Link>
          <div className="flex items-center gap-4">
            <Link href={getAdminPath(`/articles/${article.id}/revisions`)} className="inline-flex items-center gap-2 text-sm font-medium"><History className="size-4" /> Revisions</Link>
        ...
        <ArticleEditor article={article} assets={assets} adminPathPrefix={getSitePathPrefix("admin")} />
      </main>
    </AdminShell>
  );
}
